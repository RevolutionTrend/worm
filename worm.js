(function () {
		...
		page.on('response', function (response) {
			...
			if (reqUrl.includes('readSharedViewData')) {
				response.json().then((data) => {
					...
					browser.close();
					if (data.msg === 'SUCCESS') {
						generateXlsx(data.data).then((path) => {
	...
	const generateXlsx = async (data) => {
		...
		const colWidths = [
			120, 60, 85, 115, 365, 200, 312, 190, 136, 107, 135, 297, 526, 454,
			179,
		];
		...
		const writeError = await wb.write(downPath);
		// return '/doc/output.xlsx';
		return writeError ? '' : downPath;
	};

	app.get('/api/downloadFile', function (req, res) {
		res.setHeader('Content-Type', 'application/force-download');
		res.download(path.join(__dirname, 'doc/output.xlsx'));
	});
